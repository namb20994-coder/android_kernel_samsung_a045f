...
/*+S96616-2782 S96616-2785 guhan01,wt.add-time-to-full and soc95 rechg*/
...
static int get_time_to_charge_full(struct battery_data *data,struct wtchg_info *info,struct mtk_battery *gm)
{
	int magic_current;
	...
	int remain_mah = 0;
	pr_err("fgcurrent=%d\n",fgcurrent);
	fgcurrent = gauge_get_int_property(GAUGE_PROP_BATTERY_CURRENT)/10;
	int cycle_count = gm->bat_cycle;

	if (cycle_count < 299) {
		deadsoc_coefficient = DEADSOC_COEFFICIENT1;
	} else if (cycle_count < 399) {
		deadsoc_coefficient = DEADSOC_COEFFICIENT2;
	} else if (cycle_count < 699) {
		deadsoc_coefficient = DEADSOC_COEFFICIENT3;
	} else if (cycle_count < 999) {
	...
	pr_err("%s:magic_current=%d\n", __func__, magic_current);
...
/*+S96616AA3 guhan01,wt.battery cycle function*/
...
/*+S96616-2782 S96616-2785 guhan01,wt.add-time-to-full and soc95 rechg*/
			...
			time_to_full = get_time_to_charge_full(bs_data,info,gm);
			pr_err("time_to_full: %d\n", time_to_full);
			//info->time_to_full_now =  time_to_full;
...
{
/*+S96616AA3 guhan01,wt.battery cycle function*/
	extern bool N26_ic_sy6970;
	extern bool N26_ic_sgm41543;
	extern int N26_temp_cycle;
/*-S96616AA3 guhan01,wt.battery cycle function*/
	int i, cycle = 0;
	static int cycle_fv = 0;
...
/*+S96616AA3 guhan01,wt.battery cycle function*/
	if(N26_temp_cycle != -1)
	{
		cycle = N26_temp_cycle;
		pr_err("set_battery_cycle node is valid, with a value of %d\n",N26_temp_cycle);
	}
	if(N26_ic_sy6970)
	{
		if(cycle > 999)
			...
			cycle_fv = 4368000;
		pr_err("charge ic is N26_ic_sy6970,set cycle_fv=%d\n",cycle_fv);
		return cycle_fv;
	}else if(N26_ic_sgm41543){
		if(cycle > 999)
			cycle_fv = 4240000;
		else if (cycle > 699)
			cycle_fv = 4272000;
			...
			cycle_fv = 4368000;
		pr_err("charge ic is N26_ic_sgm41543,set cycle_fv=%d\n",cycle_fv);
		return cycle_fv;
	}else if (gm->batt_cycle_fv_cfg && gm->fv_levels) {
/*-S96616AA3 guhan01,wt.battery cycle function*/
		for (i = 0; i < gm->fv_levels; i += 3) {
			if ((cycle >= gm->batt_cycle_fv_cfg[i]) && (cycle <= gm->batt_cycle_fv_cfg[i + 1])) {
				chr_err("WT set cv = %d\n", gm->batt_cycle_fv_cfg[i + 2]);
